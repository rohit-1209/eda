...
def one_hot_encode_columns(table_name, columns):
    """One-hot encode categorical columns in the database table."""
    db = Database()
    conn = None
    cursor = None
    ...
    try:
        ...
        for column in columns:
            ...
            for value in unique_values:
                # Sanitize column name: replace spaces with underscore, lower case, remove problematic chars
                safe_col_name = f"{column}_{str(value).replace(' ', '_').lower()}"
                
                # Add the new column if it doesn't exist
                cursor.execute(f'''
                    ALTER TABLE "{table_name}"
                    ADD COLUMN IF NOT EXISTS "{safe_col_name}" INTEGER DEFAULT 0;
                ''')

                # Update the new column: set 1 where column == value, else 0 (default is 0 from above)
                cursor.execute(f'''
                    UPDATE "{table_name}"
                    SET "{safe_col_name}" = CASE
                        WHEN "{column}" = %s THEN 1
...
def process_categorical_features(table_name, sheet_name, operation, columns):
    """Process categorical features based on the specified operation."""
    try:
        full_table_name = f"{table_name}_copy".lower()
